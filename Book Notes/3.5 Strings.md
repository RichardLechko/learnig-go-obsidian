...
Three easy ways to convert a Integer to a String:
- Use `strconv.Itoa` which stands for "Integer to ASCII"
- Use `strconv.FormatInt` which is the same thing, but you have to specify the base which is 10
- Use `strconv.FormatUInt` which is the same thing, but you have to specify the base which is 10

You can replace the `10` with a `2` to get the numbers in binary format:
...
If you are formatting numbers solely just to print, it'd be better to use verbs in `Printf`, like `%b`, `%d`, `%u`, and `%x`.

You can do all of this in reverse - string to int:
